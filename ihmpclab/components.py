...
from ihmpclab.auxiliary import Kalman, PlotManager, Trend
...
class Model:
    ...
    def save(self, file=None, fileformat=None):
        ...
        if fileformat is 'txt':
            self.save2txt(file)
        elif fileformat is 'mat':
            self.save2mat(file)
        else:
            warn('Unknown format.')
    ...
    def step(self, nsim):
        ...
        if self._input_type is 'Incremental':
...
class ClosedLoop:
    ...
    def simulation(self, tf, u0=None, ypk=None, umin=None, umax=None, dumax=None, spec_change=None, ysp=None,
        ...
        if ymin.dtype.name is not 'object':
            if (ymin > ymax).any():
        ...
        for k in range(1, nsim):
            ...
            if self.system.input_type is "Incremental":
